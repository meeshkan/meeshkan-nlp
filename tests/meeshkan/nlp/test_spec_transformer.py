def test_optimizer(transformer, opbank_spec, opbank_recordings):
    ...
    assert (
            "random"
            == res.paths["/accounts/v3/accounts/{id}"].parameters[0]._x["x-meeshkan-id-type"]
    )

    assert (
            "random"
            == res.paths["/accounts/v3/accounts/{id}"].get._x["x-meeshkan-operation"]
    )
    ...
    assert (
            "account"
            == res.paths["/accounts/v3/accounts/{id}"]._x["x-meeshkan-entity"]
    )
