...
def test_create_object_structure():
    ...
    assert create_object_structure(obj) == [
        "properties",
        "users",
        "items",
        "properties",
...
def test_generate_child_vectors(accounts_schema):
    ...
    assert set(generate_child_vectors(accounts_schema)) == set(
        result
    )  # check equality without specific order
