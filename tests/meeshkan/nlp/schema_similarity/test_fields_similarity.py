...
def test_join_into_sentence(fields_embeddings_similarity):
    assert (
        fields_embeddings_similarity.join_into_sentence(["meeshkan", "rocks", "world"]) == "meeshkan rocks world"
    )

def test_sentence_vector(fields_embeddings_similarity):
    vector = fields_embeddings_similarity.sentence_vector(["meeshkan", "rocks", "world"])
    assert vector.shape == (300,)


def test_generate_nlp_vector(fields_embeddings_similarity):
    vector = fields_embeddings_similarity.generate_nlp_vector(["meeshkan", "rocks", "the", "world"])
    assert vector.shape == (300,)
