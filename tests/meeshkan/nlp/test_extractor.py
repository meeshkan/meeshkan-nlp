...
def test_opbank(extractor):
    ...
    assert "payment" == extractor.get_entity_from_url(
        "/v1/payments/confirm".split("/")[1:]
    )
    assert "account" == extractor.get_entity_from_url(
...
def test_transferwise(extractor):
    ...
    assert "estimate" == extractor.get_entity_from_url(
        "/v1/delivery-estimates/GcfU8g0c_pxJXR8spP3uc4jMX".split("/")[1:]
    )
    assert "account" == extractor.get_entity_from_url(
        "/v1/borderless-accounts".split("/")[1:]
    )
    assert "statement" == extractor.get_entity_from_url(
